...
|---|---|
| `kmp_exhaustive.cpp` | Every binary text through length 8 against every nonempty binary pattern through length 6, ternary cases, overlaps, empty text, embedded-NUL/high-byte cases, and seeded long cases; direct substring comparison. |
...
| `manacher_exhaustive.cpp` | Every binary string through length 10 and ternary string through length 7, empty/singleton/repeated/even palindromes, embedded-NUL/high-byte cases, and seeded cases; direct transformed-string palindrome expansion with an out-of-alphabet separator. |
| `z_value_exhaustive.cpp` | Every binary string through length 13 and ternary string through length 8, empty/singleton/repeated/periodic strings, embedded-NUL/high-byte cases, and seeded cases; direct LCP expansion from position zero. |
