...
#include "codebook/4_Flow_Matching/MincostMaxflow_dijkstra.cpp"
...
static void check(const vector<Arc> &a, int n, bool neg, int tc) {
  ...
  if (got_flow != want.flow || got_cost != want.cost) {
    cerr << "dijkstra mincost maxflow mismatch case=" << tc << " neg="
         << neg << " got=(" << got_flow << ',' << got_cost << ") want=("
         << want.flow << ',' << want.cost << ")\n";
    dump_case(a);
    exit(1);
  }
...
int main() {
  ...
  cout << "mincost_maxflow_dijkstra_exhaustive: PASS (" << cases
