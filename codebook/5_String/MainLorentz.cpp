...
void main_lorentz(const string &s, int sft = 0) {
  ...
  auto join = [](const string &a, const string &b) {
    ...
    for (unsigned char c : a) x.push_back(c); x.push_back(256);
  ...
  auto zalgo = [](const vector<int> &a) {
	...
	const auto z1 = zalgo(join(ru, "")), z2 = zalgo(join(v, u)), z3 = zalgo(join(ru, rv)), z4 = zalgo(join(v, ""));
  ...
  for (int cntr = 0; cntr < n; cntr++) {
    ...
    if (cntr < nu) {
        ...
        l = cntr - nu + 1, k1 = get_z(z3, n + nu - cntr), k2 = get_z(z4, cntr - nu + 1);
