...
\hline
\end{tabular}
$\bar{\mathbf{x}}, \bar{\mathbf{y}}$ optimal iff for all $i \in [1, n]$ either $\bar{x}_i = 0$ or $\sum_{j=1}^{m}A_{ji}\bar{y}_j = c_i$, and for all $i \in [1, m]$ either $\bar{y}_i = 0$ or $\sum_{j=1}^{n}A_{ij}\bar{x}_j = b_j$.
