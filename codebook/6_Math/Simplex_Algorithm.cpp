...
double simplex(int n, int m){
  ...
  d[n + 1][m - 1] = -1;
	...
	for (int j = 0; j < m; ++j) if (s < 0 ||
	  ix[s] > ix[j]) if (d[n + 1][j] > eps ||
	  (d[n + 1][j] > -eps && d[n][j] > eps)) s = j;
	if (s < 0) break;
	for (int i = 0; i < n; ++i) if (d[i][s] < -eps) if (r
	  < 0 ||
	  (dd = d[r][m] / d[r][s] - d[i][m] / d[i][s]) < -
	  eps || (dd < eps && ix[r + m] > ix[i + m])) r = i;
  ...
  // the missing enumerated x[i] = 0
	for (int i = m; i < n + m;
	  ++i) if (ix[i] < m - 1) ans += d[i -
	  m][m] * c[ix[i]], x[ix[i]] = d[i-m][m];
	return ans;
}
