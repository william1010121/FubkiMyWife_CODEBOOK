...
\newcommand{\inputcodedesc}[3]{
    \subsection[#1]{#1 \footnotesize{[\texttt{\captureshell{g++ -E #2 | sed '/^\#.*$/d' | tr -d '[:space:]' | md5sum | cut -c-6}}]}}%
    {\footnotesize #3\par}\vspace{-0.8em}%
    \lstinputlisting{#2}
}
% #1 title, #2 code path, #3 description, #4 example snippet path
% Keep the usage label and example at the same size as every other listing.
\newcommand{\inputcodeex}[4]{
    \inputcodedesc{#1}{#2}{#3}%
    \vspace{-1.0em}%
    {\footnotesize\itshape Usage:}\par\vspace{-0.9em}%
    \lstinputlisting[basicstyle=\footnotesize\ttfamily,aboveskip=0pt,belowskip=0pt]{#4}%
}
